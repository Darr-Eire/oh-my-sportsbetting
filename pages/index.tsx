...
export default function Home({
  todayMatches = [],
  basketballMatches = []
}: { todayMatches: LeagueBlock[]; basketballMatches: BasketballBlock[] }) {
  ...
  const toggleHorseRacing = () => {
    setOpenHorseRacing((prev) => !prev);
  };
          ...
          <section className="w-full max-w-3xl border border-gray-700 bg-[#0a1024] rounded-lg">
            ...
            >
              ...
              <span className="flex items-center gap-2">
                NBA Games
                <Image src="https://flagcdn.com/w20/us.png" alt="USA" width={20} height={14} className="object-contain rounded-sm" unoptimized />
              </span>
              ...
              <div id="basketball-matches" className="mt-3 space-y-4 px-4 pb-6 border-t border-gray-700 rounded-b-lg">
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <div className="flex gap-4 text-sm font-medium text-center">
                              <div className="flex flex-col items-center">
                                <div className="bg-gray-900 rounded px-3 py-1 text-white border border-white">{match.odds?.home?.toFixed(2) ?? "—"}</div>
                                <span className="text-xs text-softText mt-1">Home</span>
                              </div>
                              <div className="flex flex-col items-center">
                                <div className="bg-gray-900 rounded px-3 py-1 text-white border border-white">{match.odds?.away?.toFixed(2) ?? "—"}</div>
                ...
                <div className="pt-6 flex justify-center">
                  <Link href="/basketball" legacyBehavior>
                    <a className="text-sm px-5 py-2 border border-white rounded-full text-white hover:bg-cyan-600 transition font-medium">View All Basketball Games</a>
                  </Link>
                </div>
              </div>
            )}
          ...
          <section className="w-full max-w-3xl space-y-4">
                  ...
                  {openLeague === league && (
                    <div className="px-4 pb-4 pt-2 space-y-3 border-t border-gray-700 rounded-b-lg">
                      {matches.map(({ teams, time, odds }, idx) => (
                        <div key={idx} className="flex justify-between items-center bg-[#12182f] p-3 rounded-lg border border-white hover:shadow-neon transition-shadow duration-300">
                          <div>
                          ...
                          <div className="flex gap-3 text-sm font-medium text-center">
                            <div className="flex flex-col items-center">
                              <div className="bg-gray-900 rounded px-3 py-1 text-white border border-white">{odds.home.toFixed(2)}</div>
                              <span className="text-xs text-softText mt-1">Home</span>
                            </div>
                            <div className="flex flex-col items-center">
                              <div className="bg-gray-900 rounded px-3 py-1 text-white border border-white">{odds.draw?.toFixed(2) ?? "—"}</div>
                              <span className="text-xs text-softText mt-1">Draw</span>
                            </div>
                            <div className="flex flex-col items-center">
                              <div className="bg-gray-900 rounded px-3 py-1 text-white border border-white">{odds.away.toFixed(2)}</div>
                              <span className="text-xs text-softText mt-1">Away</span>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="pt-3 text-center">
                        <Link href={`/league/${league.toLowerCase().replace(/\s+/g, "-")}`} legacyBehavior>
                          <a className="text-sm text-electricCyan hover:underline font-medium">View All Matches</a>
                        </Link>
                      </div>
                    </div>
                  )}
            ...
            <div className="pt-4 flex justify-center">
              <Link href="/football" legacyBehavior>
                <a className="text-sm px-5 py-2 border border-white rounded-full text-white hover:bg-cyan-600 transition font-medium">View All Football Leagues</a>
              </Link>
            </div>
          </section>
          ...
          <section className="mt-12 w-full max-w-3xl mx-auto border border-gray-700 rounded-lg bg-[#0a1024] p-6">
            ...
            >
              {[
                ...
                <div key={idx} className="bg-[#0a1024] p-6 rounded-lg border border-gray-700 hover:shadow-neon transition-shadow duration-300 max-w-md mx-auto">
